...
const PrescriptionAll = ({ navigation }) => {
  ...
  const fetchParentSickLeaveData = async () => {
    try {
      ...
      const response = await fetch(`${BASE_URL}/api/documents/parent/prescription`, {
        method: 'GET',
        headers: {
          ...
          sickLeaveRecords.map((record, index) => (
            <View key={index} style={[styles.card, { backgroundColor: record.isUrgent ? '#FF0000' : '#000' }]}>
              <TouchableOpacity 
                ...
                onPress={() => openDocument(record.document)} 
              >
                ...
                <Text style={styles.cardTextRight}>Name:{record.Name || 'Download the PDF'}</Text>
