...
const LabRequestButtons = ({ route,navigation }) => {
  const { childData } = route.params;
  const childId = childData.id
  const [language, setLanguage] = useState('en');
  const [isModalVisible, setIsModalVisible] = useState(false);

  const SickLeave = language === 'en' ? 'Lab Reports' : 'المختبر';
  const SickLeaveRecords = language === 'en' ? 'Lab Records' : 'طلب سابق';
  const SickLeaveRequest = language === 'en' ? 'Request Lab Records' : 'طلب تحاليل المختبر';
    ...
    <Modal
      ...
      <View style={styles.buttonSection}>
        ...
        <TouchableOpacity 
          ...
          onPress={() => navigation.navigate('RequestLab')}
        >
...
export default LabRequestButtons;
