...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, TextInput, Alert , Modal, FlatList, ToastAndroid} from 'react-native';
...
const MedicalReportsRequest = ({ navigation }) => {
  ...
  const SickLeaveRequest2 = language === 'en' ? 'Sick Leave' : 'طلب تقرير طبي';
  ...
  useEffect(() => {
    const fetchChildren = async () => {
      try {
        ...
        ToastAndroid.showWithGravity(
          'Error: ' + ('An unexpected error occurred while fetching children'),
          ToastAndroid.LONG,
          ToastAndroid.BOTTOM
        );
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/api/medical-request/`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        ToastAndroid.show('Medical request submitted successfully', ToastAndroid.SHORT);
        
        navigation.goBack(); // Navigate back after successful submission
      } else {

        ToastAndroid.show('Failed to submit sick leave request', ToastAndroid.SHORT);
      }
    } catch (error) {

      ToastAndroid.show('An unexpected error occurred while submitting the request', ToastAndroid.SHORT);
    } finally {
...
export default MedicalReportsRequest;
