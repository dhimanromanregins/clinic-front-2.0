...
const RequestPrescription = ({ navigation }) => {
  ...
  const SickLeaveRequest2 = language === 'en' ? 'Request Prescription' : 'طلب وصفة طبية';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/api/prescription-request/`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        ToastAndroid.show('Prescription request submitted successfully', ToastAndroid.SHORT);
